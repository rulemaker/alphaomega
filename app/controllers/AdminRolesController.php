...
class AdminRolesController extends AdminController {
        ...
        if ($validator->passes())
        {
  	    // Get the inputs, with some exceptions
            ...
            if ($this->role->id)
            {
                ...
                return Redirect::to('roles/' . $this->role->id . '/edit')->with('success', Lang::get('admin/roles/messages.create.success'));
            }

            // Redirect to the new role page
            return Redirect::to('roles/create')->with('error', Lang::get('admin/roles/messages.create.error'));

            // Redirect to the role create page
            return Redirect::to('roles/create')->withInput()->with('error', Lang::get('admin/roles/messages.' . $error));
        }
    ...
    public function getEdit($role)
    {
        ...
        else
        {
            ...
            return Redirect::to('roles')->with('error', Lang::get('admin/roles/messages.does_not_exist'));
        }
    ...
    public function postEdit($role)
    {
        ...
        if ($validator->passes())
        {
            ...
            if ($role->save())
            {
                ...
                return Redirect::to('roles/' . $role->id . '/edit')->with('success', Lang::get('admin/roles/messages.update.success'));
            }
            else
            {
                // Redirect to the role page
                return Redirect::to('roles/' . $role->id . '/edit')->with('error', Lang::get('admin/roles/messages.update.error'));
            }
        }

        // Form validation failed
        return Redirect::to('roles/' . $role->id . '/edit')->withInput()->withErrors($validator);
    }
    ...
    public function postDelete($role)
    {
            ...
            if($role->delete()) {
                ...
                return Redirect::to('roles')->with('success', Lang::get('admin/roles/messages.delete.success'));
            }

            // There was a problem deleting the role
            return Redirect::to('roles')->with('error', Lang::get('admin/roles/messages.delete.error'));
    }
    ...
    public function getData()
    {
        ...
        ->add_column('actions', '<a href="{{{ URL::to(\'roles/\' . $id . \'/edit\' ) }}}" class="iframe btn btn-xs btn-default">{{{ Lang::get(\'button.edit\') }}}</a>
                                <a href="{{{ URL::to(\'roles/\' . $id . \'/delete\' ) }}}" class="iframe btn btn-xs btn-danger">{{{ Lang::get(\'button.delete\') }}}</a>
                    ')
